    ...
    if ($PSCmdlet.ShouldProcess('netsh', ($netshArgs -join ' '))) {
      try {
        ...
        if ($LASTEXITCODE -ne 0) {
          Write-Warning -Message ("netsh {0} failed with exit code {1}" -f ($netshArgs -join ' '), $LASTEXITCODE)
        }
      } catch {
        Write-Warning -Message ("netsh execution failed: {0}" -f $_.Exception.Message)
      }
    }
  }
